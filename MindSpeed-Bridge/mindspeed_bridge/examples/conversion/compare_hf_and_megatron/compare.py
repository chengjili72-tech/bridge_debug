...
"""
# mindspeed adapter must be first import
...
import torch
# Force disable Dynamo
torch._dynamo.config.disable = True
# Disable CUDA device compatibility check
...
# Custom bridge must be imported before using this tool, e.g.:
...
# ====================== Consolidated metrics calculation functions (global unique) ======================
stats_dict = {}


def collect_stats(x, var_name):
    """Collect tensor statistics: max, min, mean, std (compatible with NPU/CUDA/CPU/None)"""
    # Core fix: skip None values to avoid errors on non-rank-0
    if x is None:
        return
    # Handle NPU/CUDA tensors: detach gradients -> move to CPU -> convert to numpy
...
def print_stats_table_pandas(stats_dict):
    """Print a formatted statistics table using pandas"""
...
def cos_sim(x, y):
    """Calculate cosine similarity between two tensors (compatible with NPU/CUDA/CPU)"""
    # Auto-handle device/type conversion
...
def _run_hf_inference(hf_model, input_ids, pixel_values, image_grid_thw, tokenizer):
    ...
    with torch.no_grad():
        ...
        return hf_logits, hf_next_token, hf_logits_stats, hf_top5_info, logits_shape, hf_output
...
def compare_models_one_step(args) -> None:
    ...
    hf_logits, hf_next_token, hf_logits_stats, hf_top5_info, logits_shape, hf_output = _run_hf_inference(
    ...
    if torch.distributed.is_initialized():
        # Ensure consistent dtype across ranks
        if hf_logits is not None:
            hf_logits = hf_logits.float()
        # Broadcast hf_output to solve the issue of non-rank-0 being None
        if hf_output is not None:
            hf_output = hf_output.float()

        if torch.distributed.is_initialized():
            # Rank 0 gets hf_output's real last dimension (model vocabulary)
            if _is_rank_0() and hf_output is not None:
                vocab_size = hf_output.size(-1)
                vocab_tensor = torch.tensor([vocab_size], device=input_ids.device)
            if hf_output is None:
                # Critical: broadcast real vocabulary size to all ranks
                vocab_tensor = torch.zeros(1, device=input_ids.device, dtype=torch.long)
            torch.distributed.broadcast(vocab_tensor, 0)
            vocab_size = vocab_tensor.item()
        ...
        if hf_output is None:
            hf_output = torch.zeros(*input_ids.shape, len(hf_logits), device=input_ids.device, dtype=torch.float32)
        # Broadcast hf_output
        if hf_output is not None:
            rank = torch.distributed.get_rank()
            torch.distributed.broadcast(hf_output, 0)
    ...
    with torch.no_grad():
        ...
        if is_last_stage:
            ...
            if not torch.distributed.is_initialized() or parallel_state.get_tensor_model_parallel_rank() == 0:
                ...
                # ====================== Core: Full output comparison (hf_output VS megatron_output) ======================
                if parallel_state.get_tensor_model_parallel_rank() == 0 and parallel_state.is_pipeline_last_stage():
                    print("=== COMPARISON (Full Output) ===")
                    stats_dict.clear()

                    # Full output comparison
                    collect_stats(hf_output, "HF_Full_Output")
                    collect_stats(megatron_output, "Megatron_Full_Output")

                    # Full output cosine similarity
                    cosine_similarity_val = cos_sim(hf_output, megatron_output)
                    collect_stats(cosine_similarity_val, "Output_Cosine_Similarity")

                    # Print statistics table
                    print_stats_table_pandas(stats_dict)

                    # Full output difference statistics
                    diff = (hf_output - megatron_output).abs()
                    print(f"Full Output Diff - Max: {diff.max():.6f}, Mean: {diff.mean():.6f}")

                    # Original token comparison
                    token_match = hf_next_token.item() == megatron_next_token_cmp.item()
                    token_status_emoji = "✅" if token_match else "❌"
                    print(f"Next Token Match: {token_match} {token_status_emoji}")
        ...
        # Broadcast Megatron results from last rank to all ranks
        if torch.distributed.is_initialized():
