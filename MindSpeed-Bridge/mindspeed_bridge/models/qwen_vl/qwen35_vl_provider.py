...
from mindspeed_bridge.models.qwen_vl.modelling_qwen3_vl.gated_delta_net import (
    GatedDeltaNet,
...
def get_gated_delta_net_module_spec(config: TransformerConfig, backend: BackendSpecProvider = None) -> ModuleSpec:
...
def get_qwen35_block_spec(
    ...
    **Note that, this API is a experimental API in the short term, and might be deprecated in the
    ...
    if 1 in experimental_attention_pattern:
        experimental_attention_spec = get_experimental_attention_variant_module_spec(config=config, backend=backend)
    else:
...
class Qwen35VLModelProvider(GPTModelProvider):
    ...
    # =========================================================================
    transformer_layer_spec: ModuleSpec | Callable[["GPTModelProvider"], ModuleSpec] = get_qwen35_block_spec
    ...
    def provide(self, pre_process=None, post_process=None, vp_stage=None) -> Qwen3VLModel:
        ...
        block_spec = get_qwen35_block_spec(
